...
namespace cvt {
    ...
    RGBDScaleSpaceAlignment::Result RGBDScaleSpaceAlignment::alignWithKeyframe( SE3<float>& predictedPose, const MultiscaleKeyframe& kf, const Image& gray )
    {
        ...
        for( int i = _grayPyramid.octaves() - 1; i >= 0; i-- ){
            r = _alignerForOctave[ i ].alignWithKeyframe( predictedPose, kf.keyframeForOctave( i ), _grayPyramid[ i ] );
        }
        return r;
    }
