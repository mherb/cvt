...
size_t removeBorderCorners( size_t patchSize, size_t width, size_t height, std::vector<Feature2Df> & f )
{
    std::vector<Feature2Df>::iterator r, w;
    const std::vector<Feature2Df>::const_iterator end = f.end();
    w = r = f.begin();
...
int main()
{
	...
	try {
        ...
        for( size_t i = 0; i < numGood; i++ ){
            brief.descriptorForPatch( descriptors0[ i ], intImage, features0[ i ].pt );
		}
		...
		for( size_t i = 0; i < numGood; i++ ){
            ...
            brief.descriptorForPatch( descriptors1[ i ], intImage1, features1[ i ].pt );
		}
        ...
        for( size_t i = 0; i < putativeMatches.size(); i++ ){
            if( putativeMatches[ i ].size() ){
                ...
                point[ 0 ] = features0[ i ].pt.x;
                point[ 1 ] = features0[ i ].pt.y;
                homography.project( pp, point );
                std::cout << " (GT: [ " << pp[ 0 ] << ", " << pp[ 1 ] << " ] : ";
                for( size_t k = 0; k < putativeMatches[ i ].size(); k++ ){
                    std::cout << "[ " << features1[ putativeMatches[ i ][ k ] ].pt.x << ", "
                    << features1[ putativeMatches[ i ][ k ] ].pt.y << " ] ";
