...
namespace cvt {
	...
	void CLOptflow::warp( CLImage* u, CLImage* v, CLImage* px, CLImage* py, CLImage* img1, CLImage* img2, size_t iter )
	{
		...
		kernelgrad.run( cl::NullRange, img2->globalRange(), cl::NDRange( 10, 10 ), NULL, &event );
		sync.push_back( event );
		...
		while( iter-- ) {
			...
			kernelwarp.run( cl::NullRange, iwx.globalRange(), cl::NDRange( 10, 10), &sync );

			/* warp dy */
			kernelwarp.setArg( 0, &iwy );
			kernelwarp.setArg( 1, &dy );
			kernelwarp.setArg( 2, v );
			kernelwarp.run( cl::NullRange, iwy.globalRange(), cl::NDRange( 10, 10), &sync );

			/* warp dxy */
			kernelwarp.setArg( 0, &iwxy );
			kernelwarp.setArg( 1, &dxy );
			kernelwarp.setArg( 2, v );
			kernelwarp.run( cl::NullRange, iwxy.globalRange(), cl::NDRange( 10, 10), &sync );
			...
			kernelwarpsub.run( cl::NullRange, it.globalRange(), cl::NDRange( 10, 10), &sync );
...
#define PLY 10
				...
				iter = iter >> 1;
				while( iter-- ) {
					kernelth.run( cl::NullRange, ig2->globalRange(), cl::NDRange( PLX, PLY ), &sync, &event );
					sync.clear();
					sync.push_back( event );
					...
					kernelp1.run( cl::NullRange, u->globalRange(), cl::NDRange( PLX, PLY ), &sync, &event );
					sync[ 0 ] = event;
					kernelp2.run( cl::NullRange, u->globalRange(), cl::NDRange( PLX, PLY ), &sync, &event );
					sync[ 0 ] = event;
	...
	CLImage* CLOptflow::colorcode( CLImage* in )
	{
		...
		kernelcflow.run( cl::NullRange, in->globalRange(), cl::NDRange( 10, 10 ) );
		return ret;
	}
