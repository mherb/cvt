...
static void GaussIIRCoeff_D( float sigma, int order, IFilterVector4& d, float& SD )
{
	...
	d.set( -2.0f * ( el2 * cw2 + el1 * cw1 ),
		  4.0f * el1 * el2 * cw1 * cw2 + Math::sqr( el1 ) + Math::sqr( el2 ),
		  -2.0f * el1 * Math::sqr( el2 ) * cw1 - 2.0f * Math::sqr( el1 ) * el2 * cw2,
		  Math::sqr( el1 * el2 )
		 );
	SD = 1.0f + d[ 0 ] + d[ 1 ] + d[ 2 ] + d[ 3 ];
}

static void GaussIIRCoeff_N( float sigma, int order, IFilterVector4& n, float& SN )
{
	const float cw1 = Math::cos( W1 / sigma );
	const float cw2 = Math::cos( W2 / sigma );
	...
	n.set( A1 + A2,
		   el2 * ( B2 * sw2 - ( A2 + 2.0f * A1 ) * cw2 ) + el1 * ( B1 * sw1 - ( A1 + 2.0f * A2 ) * cw1 ),
		   2.0f * el1 *el2 * ( ( A1 + A2 ) * cw2 * cw1 - B1 * cw2 * sw1 - B2 * cw1 * sw2  ) + A2 * Math::sqr( el1 ) + A1 * Math::sqr( el2 ),
		   Math::sqr( el1 ) * el2 * ( B2 *sw2 - A2 * cw2 ) + el1 * Math::sqr( el2 ) * ( B1 * sw1 - A1 * cw1 )
		  );

	SN = n[ 0 ] + n[ 1 ] + n[ 2 ] + n[ 3 ];
}

void GaussIIRCoeff( float sigma, int order, IFilterVector4& n, IFilterVector4& m, IFilterVector4& d )
{
