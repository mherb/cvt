...
namespace cvt {
    ...
    inline float MITracker::MI()
    {
        ...
        for( size_t y = 0; y < _numBins+1; y++ ) {
            for( size_t x = 0; x < _numBins+1; x++ ) {
                ...
                mi += jval * Math::log( ( jval ) / ( _thist[ y ] * _whist[ x ] + 1e-10f ) );
            }
        }
    ...
    inline void MITracker::updateApproxHessian()
    {
		...
		_miHessian /= Math::sqr( MI() );
		updateDerivatives();
		_miHessian -= ( 2.0f / ( pow( MI(), 3.0f )  ) ) *_miJacobian * _miJacobian.transpose();
        _miHessian = hessSum.inverse();
        std::cout << "Hessian: " << _miHessian << std::endl;
