...
namespace cvt
{
    ...
    template <typename T>
    inline void PoseTranslation<T>::screenJacobian( PoseTranslation<T>::ScreenJacType& j,
    ...
    template <typename T>
    inline void PoseTranslation<T>::screenHessian( PoseTranslation<T>::ScreenHessType& hx,
                                                   PoseTranslation<T>::ScreenHessType& hy,
                                                   const Eigen::Matrix<T, 2, 1>& ) const
    {
        hx.setZero();
        hy.setZero();
    ...
    template <typename T>
    inline void PoseTranslation<T>::apply( const PoseTranslation<T>::ParameterVectorType& delta )
    {
    ...
    template <typename T>
    inline void PoseTranslation<T>::applyInverse( const PoseTranslation<T>::ParameterVectorType& delta )
    {
        _current( 0 , 2 ) += delta[ 0 ];
        _current( 1 , 2 ) += delta[ 1 ];
    }
