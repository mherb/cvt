...
namespace cvt
{
	...
	void AGAST::agast7_12s( const uint8_t* im, size_t stride, size_t xsize, size_t ysize, std::vector<Feature2Df> & corners )
	{
		register int y;
		register int x;
...
int* AGAST::score7_12s( const uint8_t* i, size_t stride, std::vector<Feature2Df> & corners )
{
    ...
    for(size_t n = 0; n < corners.size(); n++)
        scores[n] = score_agast7_12s( i + (int)corners[ n ].pt.y * stride + (int)corners[ n ].pt.x );
